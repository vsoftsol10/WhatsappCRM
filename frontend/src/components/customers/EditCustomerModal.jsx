...
export default function EditCustomerModal({
  ...
  useEffect(() => {
    ...
    const fetchCustomer = async () => {
      try {
        ...
        // strip leading "91" or "+91" so the 10-digit validation passes
        const cleanPhone = (data.customer.phone || "").replace(/^(\+?91)/, "");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // re-add the "91" prefix so it's stored in DB the same way as before
      const payload = {
        ...formData,
