...
const { ALLOWED_PRODUCTS } = require("../utils/classificationPromptBuilder");
...
// { product, confidence, summary } so Step 5 (Product Router) can
// decide which team the lead belongs to.
// Never throws — on any failure (bad JSON, API error, etc.) it falls
// back to a safe "Other" result so the webhook flow never breaks
...
const classifyCustomerMessage = async (messageText) => {
  ...
  try {
    ...
    return { product, confidence, summary };
  } catch (error) {
