...
// const { getAiSettings } = require("../services/aiSettingsService");
...
//         model: classification?.model ?? "grok-4-fast",
//       });

//       // Groq was down/overloaded for every retry — don't let this
...
//             message: `Groq was unreachable for a WhatsApp message from ${phone}: "${text}". Please review this conversation manually.`,
...
//       // Auto-reply (Groq, conversational fallback): several paths above
//       // (product "Other"/low-confidence → no lead created; an
//       // already-fully-enriched lead's follow-up message; a SUPPORT
//       // ticket that was deduped/silently forwarded) legitimately don't
//       // send the customer anything. Rather than leaving them with
//       // silence, check whether *any* outgoing message has already gone
//       // out for this turn — if not, and the bot is allowed to talk in
//       // this conversation, let Groq write a context-aware reply.
//       try {
//         const globalSettings = await getAiSettings();

//         if (conversation.botEnabled !== false && globalSettings.autoReplyEnabled !== false) {
//           const alreadyReplied = await prisma.message.count({
//             where: {
//               conversationId: conversation.id,
//               sender: "AGENT",
//               createdAt: { gt: savedMessage.createdAt },
//             },
//           });
...
//             if (autoReplyText) {
//               await sendAndSaveOutgoingMessage(conversation, autoReplyText);
//             }
//           }
//         }
//       } catch (autoReplyError) {
//         console.error("Groq auto-reply error:", autoReplyError);
//       }
//       }
//     }
  ...
  try {
    ...
    if (statuses && statuses.length > 0) {
      for (const statusEvent of statuses) {
        ...
        const upperStatus = newStatus ? newStatus.toUpperCase() : undefined;
        ...
        // Campaign and Template recipient rows track their own
        // status/deliveredAt/readAt so the Campaign/Template UI can show
        // per-recipient delivery status without joining through Message.
        // Matched back by the same metaMessageId stored at send time.
        if (upperStatus) {
          const recipientUpdateData = {
            status: upperStatus,
            failureReason,
            ...(upperStatus === "DELIVERED" && { deliveredAt: new Date() }),
            ...(upperStatus === "READ" && { readAt: new Date() }),
          };
          ...
          try {
            ...
            await prisma.templateRecipient.updateMany({
              where: { metaMessageId },
              data: recipientUpdateData,
            });

            getIO().to("agents").emit("recipient:status", {
              metaMessageId,
              status: upperStatus,
              failureReason,
            });
          } catch (err) {
            console.error("Failed to update recipient status:", err);
          }
        }
      }
    }
