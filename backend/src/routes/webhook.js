...
const processWebhookPayload = async (value) => {
  try {
    ...
    if (message) {
      ...
      // Grok was down/overloaded for every retry — don't let this
      // message quietly disappear as if it were "hi" with nothing to
      // say. Alert admins so a human can check the conversation and
      // create the lead/ticket manually if it turns out to be a real
      // enquiry.
      if (classification?.aiUnavailable) {
        try {
          await notifyAdmins({
            title: "AI classification unavailable",
            message: `Grok was unreachable for a WhatsApp message from ${phone}: "${text}". Please review this conversation manually.`,
            type: NotificationType.SYSTEM,
          });
        } catch (notifyError) {
          console.error("Failed to notify admins of AI outage:", notifyError);
      ...
      // Auto-reply (Grok, conversational fallback): several paths above
      ...
      // this conversation, let Grok write a context-aware reply.
      try {
        ...
        console.error("Grok auto-reply error:", autoReplyError);
      }
      }
    }
