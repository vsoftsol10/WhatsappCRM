...
try {
  ...
  // side — no {{2}} content parameter, since WhatsApp strips
  // newlines out of template parameter values and that's exactly
  // why generic-template campaigns lose their formatting. Falls
  // back to the generic templates when no dedicated one is set.
  ...
  if (campaign.imageUrl) {
    ...
    result = await sendCampaignImageTemplate(
      ...
      usesDedicatedMetaTemplate
       ...
       campaign.imageUrl,
      usesDedicatedMetaTemplate
        ? [] // fully static body on Meta's side — only the header image is dynamic
    ...
    result = await sendTemplateMessage(
      ...
      usesDedicatedMetaTemplate
        ? [customer.name] // adjust if this template's body has more variables
        : [customer.name, personalizedMessage] // fills {{1}} and {{2}}
    );
