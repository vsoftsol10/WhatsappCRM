...
if (req.file) {
  ...
  type: "CAMPAIGN",
    ...
    await recordAuditLog({
      ...
      details: `${campaign.name} (${campaign.audienceCount} recipients)`,
...
if (req.file) {
  ...
  imageUrl = uploadResult?.imageUrl || existingCampaign.imageUrl;
    ...
    if (name && name !== existingCampaign.name) {
      campaignChanges.push(`name: ${existingCampaign.name} -> ${name}`);
    }
    if (status && status !== existingCampaign.status) {
      campaignChanges.push(`status: ${existingCampaign.status} -> ${status}`);
    }
    if (type && type !== existingCampaign.type) {
      campaignChanges.push(`type: ${existingCampaign.type} -> ${type}`);
    }
    ...
    if (campaignChanges.length > 0) {
      await recordAuditLog({
        ...
        entityType: "Campaign",
        entityId: campaign.id,
        details: `${existingCampaign.name}: ${campaignChanges.join(", ")}`,
        actorId: req.user?.userId,
      });
    }
