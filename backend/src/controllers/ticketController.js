...
const createTicket = async (req, res) => {
  try {
    ...
    await recordAuditLog({
      ...
      entityType: "Ticket",
      entityId: ticket.id,
      details: `${ticket.title} (customer: ${ticket.customer?.name || customerId})`,
...
const updateTicket = async (req, res) => {
  try {
    ...
    if (title !== undefined && title !== existingTicket.title) {
      ticketChanges.push(`title: ${existingTicket.title} -> ${title}`);
    }
    if (priority !== undefined && priority !== existingTicket.priority) {
      ticketChanges.push(`priority: ${existingTicket.priority} -> ${priority}`);
    }
    if (status !== undefined && status !== existingTicket.status) {
      ticketChanges.push(`status: ${existingTicket.status} -> ${status}`);
    }
    if (
      ...
      ticketChanges.push(
        `assignedTo: ${existingTicket.assignedToId || "unassigned"} -> ${
    ...
    if (ticketChanges.length > 0) {
      await recordAuditLog({
        ...
        details: `${existingTicket.title}: ${ticketChanges.join(", ")}`,
        actorId: req.user?.userId,
      });
    }
...
const updateTicketStatus = async (req, res) => {
  try {
    ...
    await recordAuditLog({
      ...
      entityType: "Ticket",
      entityId: updatedTicket.id,
      details: `${updatedTicket.title}: ${ticket.status} -> ${status}`,
      actorId: req.user?.userId,
    });
