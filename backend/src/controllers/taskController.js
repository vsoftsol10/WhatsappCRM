...
const createTask = async (req, res) => {
  try {
    ...
    await recordAuditLog({
      ...
      entityType: "Task",
      entityId: task.id,
      details: `${task.title} (assigned to ${task.assignedTo?.name || "unassigned"})`,
...
const updateTask = async (req, res) => {
  try {
    ...
    if (title !== undefined && title !== existingTask.title) {
      taskChanges.push(`title: ${existingTask.title} -> ${title}`);
    }
    if (priority !== undefined && priority !== existingTask.priority) {
      taskChanges.push(`priority: ${existingTask.priority} -> ${priority}`);
    }
    if (
      ...
      taskChanges.push(
        `assignedTo: ${existingTask.assignedToId} -> ${assignedToId}`
      );
    }
    ...
    if (taskChanges.length > 0) {
      await recordAuditLog({
        ...
        entityType: "Task",
        entityId: task.id,
        details: `${existingTask.title}: ${taskChanges.join(", ")}`,
        actorId: req.user?.userId,
      });
    }
...
const updateTaskStatus = async (req, res) => {
  try {
    ...
    await recordAuditLog({
      ...
      details: `${updatedTask.title}: ${task.status} -> ${status}`,
      actorId: req.user?.userId,
    });
