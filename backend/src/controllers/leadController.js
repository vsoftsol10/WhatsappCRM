...
const sendLeadToErp = async (req, res) => {
  try {
    ...
    // Same ownership rule as update/status/convert/delete on this
    // controller — a non-admin employee shouldn't be able to push a
    // lead assigned to someone else out to the ERP-CRM.
    if (
      req.user.role !== "ADMIN" &&
      lead.assignedToId !== req.user.userId
    ) {
      return res.status(403).json({
        success: false,
        message: "You do not have permission to send this lead to ERP-CRM",
      });
    }
